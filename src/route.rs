...
impl Route {
    fn path(&self, data: &Data, base: &str) -> String {
        match self {
            ...
            Route::TodoComplete(q) => {
                format!(
                    "{base}/#todo/{}/complete",
                    encode(data.quest_locale.get_url(*q))
                )
            }
            Route::TodoAction(q, l) => format!(
                "{base}/#todo/{}/{}",
                encode(data.quest_locale.get_url(*q)),
                encode(data.quest_locale.location(*l)),
            ),
            Route::Map => format!("{base}/#map"),
            Route::MapLocation(l) => {
                format!("{base}/#map/{}", encode(data.quest_locale.location(*l)))
            }
            ...
            Route::MapNewQuest(l) => {
                format!("{base}/#map/{}/new", encode(data.quest_locale.location(*l)))
            }
            Route::Edit => format!("{base}/#edit"),
            Route::EditQuest(q) => {
                format!("{base}/#edit/{}", encode(data.quest_locale.get_url(*q)))
            }
    ...
    fn parse(data: &Data, hash: &str) -> Self {
        ...
        match path.next() {
            Some("todo") => {
                let q = path.next().and_then(|q| data.quest_locale.try_get_url(q));
                let p2 = path.next();
                let l = p2.and_then(LocationId::try_from_name);
                let c = if p2 == Some("complete") {
                    Some(())
                } else {
                    None
                };
                if let Some((q, ())) = q.zip(c) {
                    Route::TodoComplete(q)
                } else if let Some((q, l)) = q.zip(l) {
            ...
            Some("map") => {
                ...
                if let Some(l) = l {
                    let snd = path.next();
                    let q = snd.and_then(|q| data.quest_locale.try_get_url(q));
                    if let Some(q) = q {
                        Route::MapAction(l, q)
