...
use crate::game::{GameLanguage, LocationId, QuestId, QuestType};
...
impl Data {
    pub(crate) fn save_csv(&self) -> Vec<u8> {
        ...
        for (_, quest, quest_name) in self.quest_iter() {
                ...
                || quest.vis != Vis::Visible
            {
                let _ = writer.write_record([
                    ...
                    "",
                    quest.vis.to_csv(),
                    if quest.completed { "completed" } else { "" },
                    &quest.note,
                ]);
            }
        }
        for location_id in LocationId::all() {
            if location_id != LocationId::prologue() {
                if let Some(location) = self.location.get(&location_id) {
                    if !location.note.is_empty() || location.removed {
                        let _ = writer.write_record([
                            "location",
                            location_id.name(self.quest_locale.language()), // language does not anyway matter as this is never prologue
                            "",
                            if location.removed { "removed" } else { "" },
                            "",
                            "",
                            "",
                            &location.note,
    ...
    pub(crate) fn load_csv(&mut self, mut file: &[u8]) -> Result<String, MyError> {
        ...
        let quests = self
            .quest_locale
            .all_str()
            .filter(|(a, _)| a.typ() != QuestType::Unavailable)
            .map(|(a, b)| (b, a))
            .collect();

        while reader.read_record(&mut line)? {
            if self.load_csv_line(&line, &locations, &quests).is_none() {
    ...
    fn load_csv_line(
        ...
        let note = line.get(7).unwrap_or_default();
        match type_ {
            Type::Quest => {
                let quest_id = quest_id?;
                let state = QuestState::try_from_csv(line.get(3).unwrap_or_default())?;
                let completed = match line.get(6).unwrap_or_default() {
                    "" => false,
                    "completed" => true,
                    _ => return None,
                };
            ...
            Type::Location => {
                ...
                let removed = match line.get(6).unwrap_or_default() {
                    "" => false,
                    "removed" => true,
                    _ => return None,
                };
