...
impl SubComponent for PaneMapNewQuest {
    ...
    fn update(
        ...
        match msg {
            ...
            MsgMapNewQuest::SelectQuest(quest_id) => {
                if let Some(replaced_quest_id) = quest_id.replacement() {
                    self.quest_id = replaced_quest_id;
                } else if self
                    .location_id
                    .name(data.quest_locale.language())
                    .starts_with('N')
                    && quest_id == QuestId::quest_lost()
                {
                    self.quest_id = QuestId::quest_confused();
                } else {
                    self.quest_id = quest_id;
                }
                self.prerequisite = None;
                self.hint_lost_confused = self.quest_id == QuestId::quest_confused();
    ...
    fn view(&self, data: &Data, ctx: &Context<App>) -> Html {
        match self.page {
            ...
            Page::NoteAndPrerequisite => {
                if self.quest_id.typ() == QuestType::Unavailable {
                    self.view_new_quest3_unavailable(data, ctx)
                } else if !nqt_allow(self.encounter_type, data, self.quest_id) {
                    Self::view_new_quest3_duplicate(data, ctx)
                } else {
                    self.view_new_quest3(data, ctx)
                }
            }
        }
    }
...
impl PaneMapNewQuest {
    ...
    pub(crate) fn view_new_quest(&self, data: &Data, ctx: &Context<App>) -> Html {
        html! {
            <ul class="list-group">
                ...
                <li class="list-group-item">
                    {data.msg.location(data.quest_locale.location(self.location_id))}
                </li>
                <li class="list-group-item">
    ...
    pub(crate) fn view_new_quest2(&self, data: &Data, ctx: &Context<App>) -> Html {
        let quests: Vec<_> = if self.encounter_type == EncounterType::Gain {
            QuestId::cards().map(|(quest_id, card_name)| {
                html! {
                    <li class="list-group-item">
                        <Button style={Color::Primary} onclick={ctx.link().callback(move |_|MsgMapNewQuest::SelectQuest(quest_id))} children={text(card_name)} />
                    </li>
                }
            }).collect()
        } else {
            data.quest_all_iter().filter_map(|(quest_id, quest_name)| {
                if quest_id.typ() == QuestType::Keyword && nqt_allow(self.encounter_type, data, quest_id) {
                    Some(html! {
                        <li class="list-group-item">
    ...
    pub(crate) fn view_new_quest3_unavailable(&self, data: &Data, ctx: &Context<App>) -> Html {
        html! {
            <ul class="list-group">
                <li class="list-group-item">
                    <Button style={Color::Primary} onclick={ctx.link().callback(|_|MsgApp::Back)} children={data.msg.back()} />
                ...
                <li class="list-group-item">
                    {data.msg.ma_nq_cant_track(html!{<b>{text(data.quest_locale.get(self.quest_id))}</b>})}
                </li>
            </ul>
        }
    }

    pub(crate) fn view_new_quest3_duplicate(data: &Data, ctx: &Context<App>) -> Html {
        html! {
            <ul class="list-group">
                <li class="list-group-item">
                    <Button style={Color::Primary} onclick={ctx.link().callback(|_|MsgApp::Back)} children={data.msg.back()} />
                ...
                <li class="list-group-item">
                    {data.msg.ma_nq_already_tracked()}
                </li>
            </ul>
        }
    }
    ...
    pub(crate) fn view_new_quest3(&self, data: &Data, ctx: &Context<App>) -> Html {
        ...
        html! {
            <ul class="list-group">
                ...
                if self.hint_lost_confused {
                    <li class="list-group-item">
                        <strong>{data.msg.ma_nq_hint_lost_confused()}</strong>
                    </li>
                }
