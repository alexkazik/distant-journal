...
pub(crate) struct PaneEditQuest {
    ...
    cb_toggle_completed: Callback<Event>,
...
impl SubComponent for PaneEditQuest {
    ...
    fn create(ctx: &Context<App>) -> Self {
        Self {
            ...
            cb_toggle_completed: ctx.link().callback(|_| MsgEditQuest::ToggleFinished),
    ...
    fn view(&self, data: &Data, ctx: &Context<App>) -> Html {
                ...
                quest_location.iter().map(move |(encounter_type, encounter)| {
                    ...
                    html! {
                        <li class="list-group-item">
                            {data.msg.location_with_icon(data.quest_locale.location(location_id), encounter_type.icon_active())}<br/>
        ...
        html! {
            ...
            <ul class="list-group">
                <li class="list-group-item">
                    {data.msg.quest_header(data.quest_locale.get(self.quest_id), self.quest_id.icon())}
                </li>
                <li class="list-group-item">
                ...
                if self.quest_id.typ() != QuestType::Keyword {
                    <li class="list-group-item">
                        <div class="form-check form-switch">
                            <input
                                ...
                                role="switch"
                                id="completed"
                                checked={self.quest.completed}
                                onchange={&self.cb_toggle_completed}
                            />
                            <label class="form-check-label" for="completed">{data.msg.ed_qu_completed_quest()}</label>
                        </div>
                    </li>
                }
                <li class="list-group-item">
