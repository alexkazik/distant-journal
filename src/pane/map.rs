    ...
    fn update(
        ...
        match msg {
            ...
            MsgMap::ShowNote(location_id) => {
                if let Some(location) = data.location.get(&location_id) {
                    self.modal.open(
                        &data
                            .msg
                            .str_note_modal_head(data.quest_locale.location(location_id)),
                        &location.note,
                    );
                }
                UpdateResult::empty()
    ...
    fn view(&self, data: &Data, ctx: &Context<App>) -> Html {
        let map = MAP.iter().enumerate().map(|(pos, pl_pr)| {
            match pl_pr {
                None => html! {<td/>},
                Some((pl, pr)) => {
                    let page = if pr.is_empty() { data.msg.page_short(pl) } else { data.msg.page_short_lr(pl, pr) };
                    let style = if self.position == pos { Color::Success } else { Color::Warning };
                    html! {<td width="25%" align="center"><Button size={ButtonSize::Small} style={style} onclick={ctx.link().callback(move |_| MsgMap::Page(pos))}>{page}</Button></td>}
                }
            }
        }).collect::<Vec<_>>();
        let map = map.chunks(4).map(|tr| {
...
impl PaneMap {
    fn render_list(data: &Data, ctx: &Context<App>, page: &str) -> Html {
        let locations = LocationId::all().filter(|l| l.page() == Some(page))
            .map(|l| {
                let location = data.location.get(&l);
                ...
                html! {
                    <tr>
                        ...
                        <td>
                        if let Some(note) = location.and_then(|l|if l.note.is_empty(){None}else{Some(&l.note)}) {
                            {" "}
                            <Button
                              class="btn-sm d-flex align-items-center"
                              style={Color::Info}
