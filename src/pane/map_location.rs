...
impl SubComponent for PaneMapLocation {
    ...
    fn update(
        ...
        match msg {
            MsgMapLocation::Note(str) => {
                data.location.entry(self.location_id).or_default().note = str.into();
            ...
            MsgMapLocation::ToggleRemoved => {
                data.location.entry(self.location_id).or_default().removed ^= true;
                UpdateResult::SaveGameData.into()
            }
        }
    }
    ...
    fn view(&self, data: &Data, ctx: &Context<App>) -> Html {
        ...
        let location = data.location.get(&self.location_id);
        let location_note = location.map(|l| l.note.to_string());
        let quests = data
            .quest_iter()
