    ...
    fn update(&mut self, data: &mut Data, ctx: &Context<App>, msg: Self::Message) -> UpdateResults {
        match msg {
            ...
            MsgSettings::MsgLanguage(language) => {
                if let Some((ql, _)) = MsgLanguage::iter()
            ...
            MsgSettings::NewCampaign => {
                for (quest_id, quest) in &mut data.quest {
                    if QuestId::STARTING_QUESTS.contains(quest_id) {
                ...
                for location in data.location.values_mut() {
        ...
        T: Copy + PartialEq,
    {
        ...
        html! {
            <li class="list-group-item">
                ...
                <br/>
                <a href="https://github.com/alexkazik/distant-journal#Translation" target="_blank">{data.msg.sett_translation_help()}</a>
                if incomplete {
