...
use crate::html::callback_input_value;
...
impl SubComponent for PaneAction {
    ...
    fn create(_ctx: &Context<App>) -> Self {
        Self {
            quest_id: QuestId::STARTING_QUESTS[0], // any value
    ...
    fn view(&self, data: &Data, ctx: &Context<App>) -> Html {
        ...
        let enc = quest_location_iter
            ...
            .map(|(encounter_type, qle)| {
                ...
                let message = et2msg(data, encounter_type, quest, active);
                let message = if let Some(message) = message {
                    html! {
                        <>
                            if active {
                                <Button style={Color::Success} onclick={ctx.link().callback(move |_|MsgAction::Perform(encounter_type))} children={message} />
                            } else {
                                <Button style={Color::Success} outline={true} children={message} />
                            }
                            <br/>
                        </>
                        }
                ...
                if self.is_map {
                    html! {
                        <li class="list-group-item">
                            {data.msg.location_with_icon(data.quest_locale.location(self.location_id),  encounter_type.icon(active))}<br/>
                            {message}
...
fn et2msg(data: &Data, encounter_type: EncounterType, quest: &Quest, active: bool) -> Option<Html> {
    match encounter_type {
        ...
        EncounterType::Gain => {
            if active {
                Some(data.msg.acti_gain_quest())
            } else if quest.state != QuestState::NotFound {
                Some(data.msg.acti_gain_quest_found())
            } else {
                Some(data.msg.acti_gain_quest_no_pre())
            }
        }
        EncounterType::Complete => {
            if active {
                Some(data.msg.acti_complete_quest())
            } else {
                Some(data.msg.acti_complete_quest_no())
            }
        }
        EncounterType::Lose => {
            if active {
                Some(data.msg.acti_lose_quest())
            } else {
                Some(data.msg.acti_lose_quest_no())
            }
        }
    }
}
